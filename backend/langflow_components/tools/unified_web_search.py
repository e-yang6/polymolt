...
class WebSearchComponent(Component):
    ...
    def perform_web_search(self) -> DataFrame:
        ...
        try:
            response = requests.get("https://html.duckduckgo.com/html/", params=params, headers=headers, timeout=self.timeout)
        ...
        if not response.text or "text/html" not in response.headers.get("content-type", "").lower():
            ...
            return DataFrame(pd.DataFrame([{"title": "Error", "link": "", "snippet": "No results found", "content": ""}]))
    ...
    def perform_news_search(self) -> DataFrame:
        ...
        if topic:
            rss_url = f"https://news.google.com/rss/headlines/section/topic/{quote_plus(topic.upper())}?hl={hl}&gl={gl}&ceid={ceid}"
        elif location:
            rss_url = f"https://news.google.com/rss/headlines/section/geo/{quote_plus(location)}?hl={hl}&gl={gl}&ceid={ceid}"
        elif query:
            rss_url = f"https://news.google.com/rss/search?q={quote_plus(query)}&hl={hl}&gl={gl}&ceid={ceid}"
        else:
            self.status = "No search query, topic, or location provided."
            return DataFrame(pd.DataFrame([{"title": "Error", "link": "", "published": "", "summary": "No search parameters provided"}]))
        try:
    ...
    def perform_rss_read(self) -> DataFrame:
        ...
        return DataFrame(pd.DataFrame(articles, columns=["title", "link", "published", "summary"]))

    def perform_search(self) -> DataFrame:
        mode = getattr(self, "search_mode", "Web")
        if mode == "News":
